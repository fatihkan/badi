...
export function detectDrift(filePath, opts = {}) {
	return detectDriftSource(readFileSync(filePath, "utf-8"), opts);
}

/**
 * Drift detection over a raw source string (file content already read). Lets
 * directory-modules be audited by combining their index.js dispatch with their
 * help.js help text — see auditFiles.
 *
 * @param {string} source
 * @param {object} [opts]
 * @returns {{ missingSubs: string[], missingFlags: string[] }}
 */
export function detectDriftSource(source, opts = {}) {
...
export function auditFiles(filePaths, opts = {}) {
	...
	for (const filePath of filePaths) {
		...
		// Directory-module (e.g. mobile/, icerik/): audit the index.js dispatch
		// against the help.js help text combined, so split commands stay covered
		// (a plain .js filter would silently drop them from help-doctor).
		let drift;
		if (existsSync(filePath) && statSync(filePath).isDirectory()) {
			const idx = join(filePath, "index.js");
			const help = join(filePath, "help.js");
			let source = existsSync(idx) ? readFileSync(idx, "utf-8") : "";
			if (existsSync(help)) source += `\n${readFileSync(help, "utf-8")}`;
			drift = detectDriftSource(source, { allowSubs, allowFlags });
		} else {
			drift = detectDrift(filePath, { allowSubs, allowFlags });
		}
