...
export const VERSION = JSON.parse(readFileSync(resolve(PKG_ROOT, "package.json"), "utf-8")).version;
...
try {
	...
	const identity = (s) => s;
	const handler = {
		get() { return new Proxy(identity, handler); },
		apply(_target, _thisArg, args) { return args[0]; },
	};
