...
// The tool-name vocabulary a PreToolUse/PostToolUse matcher may reference.
// A matcher naming anything else fires on nothing — a hook that is present but
// dead, the same silent-outage class as an invalid SessionStart source. Kept in
// sync with lib/harnesses/qwen.js TOOL_MAP's Claude-side keys.
...
/**
 * Find PreToolUse/PostToolUse/PostToolUseFailure matchers that reference a tool
 * name outside CLAUDE_TOOLS. Such a matcher is wired but can never fire — the
 * hook is "present but dead," distinct from a missing hook file. Empty and "*"
 * matchers (match-all) are always live. Pure + exported for testing.
 *
 * @param {object} settings  parsed settings.json
 * @returns {string[]}  the offending "event: matcher" strings (empty = all live)
 */
export function deadToolMatchers(settings) {
	const out = [];
...
export default {
	...
	doctor({ target }) {
		...
		run("hook matchers name live tools (not present-but-dead)", () => {
			// A PreToolUse/PostToolUse matcher naming a tool that doesn't exist
			// fires on nothing — the hook is wired but dead. Distinct from a
			// missing hook file (checked below), which is why it's its own check.
			const p = join(claudeDir, "settings.json");
			if (!existsSync(p)) return "warn";
			let settings;
			try {
			...
			return deadToolMatchers(settings).length === 0 ? true : "warn";
		});
