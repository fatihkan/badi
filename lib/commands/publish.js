...
export async function runPublish(args) {
	...
	// Plugin manifest sync (v1.32+, issue #196)
	// lastUpdated, package.json bump commit'iyle senkron olsun diye su anki tarih
	// pass edilir. Sonraki commit'te lastPackageJsonCommitDate() bunu match eder.
	...
	if (!flags.skipManifestSync && pluginDirExists) {
		...
		if (flags.dryRun) {
			...
			try {
				...
				console.log(`  ${chalk.red("XX")} Manifest sync basarisiz: ${e.message}`);
				console.log(
					chalk.dim(
		...
		console.log(
			chalk.dim("  Sonradan manuel: badi release sync-manifest"),
		);
		console.log("");
	}
