...
function validateUrl(url) {
	...
	if (!["http:", "https:"].includes(parsed.protocol)) {
		throw new Error(
			`Sadece http/https URL'leri desteklenir: ${parsed.protocol}`,
		);
	}
...
async function fetchPage(url) {
	...
	try {
		const res = await fetch(url, {
			...
			headers: {
				"User-Agent": "Badi-SEO/1.4 (+https://github.com/fatihkan/badi)",
			},
			redirect: "follow",
		});
...
async function fetchDDG(query, timeoutMs = 15000) {
	...
	try {
		...
		clearTimeout(timer);
		throw new Error(`DDG fetch: ${e.message}`);
	}
}
...
async function seoBacklinks(domain) {
		...
		while ((m = regex.exec(html)) !== null) {
			try {
				...
				if (host !== clean && !host.endsWith(`.${clean}`)) {
					referring.set(host, (referring.get(host) || 0) + 1);
				}
			} catch {
				/* gecersiz URL, atla */
			}
	...
	try {
		...
		const res = await fetch(cdxUrl, {
			headers: { "User-Agent": "Badi-SEO/1.11" },
		});
		if (res.ok) {
