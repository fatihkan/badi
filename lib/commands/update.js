...
export function runUpdate(args, { showHelp }) {
	...
	if (!existsSync(destClaude)) {
		console.error(
			chalk.red(
				"Hata: .claude/ dizini bulunamadi. Once 'badi init' calistirin.",
			),
		);
		process.exit(1);
	}
	...
	if (existsSync(srcClaudeMd)) {
		...
		if (claudeMdMissing || force) {
			...
			console.log(
				`  ${chalk.green(claudeMdMissing ? "+" : "!")} CLAUDE.md ${claudeMdMissing ? "(yeni eklendi)" : "(ustune yazildi)"}`,
			);
			result.copied++;
		}
	}
	...
	if (force) {
		...
		console.log(
			`  ${chalk.gray(result.skipped)} kullanici dosyasi korundu (memory/knowledge/workspace)`,
		);
	} else {
