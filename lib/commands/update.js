...
export function runUpdate(args, { showHelp }) {
	...
	// CLAUDE.md guncelle (mevcut degilse veya paket surumu daha yeniyse)
	const srcClaudeMd = join(PKG_ROOT, "CLAUDE.md");
	const destClaudeMd = join(target, "CLAUDE.md");
	if (existsSync(srcClaudeMd) && !existsSync(destClaudeMd)) {
		if (!dryRun) cpSync(srcClaudeMd, destClaudeMd);
		console.log(`  ${chalk.green("+")} CLAUDE.md (yeni eklendi)`);
		result.copied++;
	}
