...
export function runDoctor(args, { showHelp }) {
	...
	const expectedHooks = [
		"guard-bash.sh", "backup-before-write.sh", "completeness-gate.sh",
		"log-changes.sh", "log-failures.sh", "log-stop-verdict.sh",
		"post-compact-resume.sh", "pre-compact-handoff.sh", "session-reset.sh",
		"dependency-audit.sh", "branch-guard.sh", "track-usage.sh",
	];
	...
	const expectedAgents = [
		"archaeologist", "auditor", "coach", "debt-collector", "error-whisperer",
		"onboarding-sherpa", "pr-ghostwriter", "rubber-duck", "unsticker", "yak-shave-detector",
		"security-scanner", "performance-profiler", "test-strategist", "api-designer",
		"migration-pilot", "code-generator", "refactoring-advisor", "architecture-advisor",
		"content-creator", "visual-director", "project-architect",
	];
	...
	if (fail === 0 && warn === 0) {
		...
		console.log(`  ${chalk.cyan("badi update")}         # Eksik dosyalari ekler, ozel dosyalari korur (onerilen)`);
		console.log(`  ${chalk.cyan("badi init --force")}   # Her seyi zorla yeniden kurar (ozel degisiklikler kaybolur)`);
		console.log("");
		console.log(chalk.dim("Hook eksikse su hatayi alirsiniz:"));
		console.log(chalk.dim("  bash: .claude/hooks/guard-bash.sh: No such file or directory"));
