...
export async function runLighthouse(args) {
	...
	if (!url || ["--help", "-h"].includes(url)) {
		...
		console.log("");
		console.log(`  ${chalk.cyan("badi lighthouse")} [url]               Mobile audit (varsayilan)`);
		console.log(`  ${chalk.cyan("badi lighthouse")} [url] --desktop     Desktop audit`);
		console.log("");
	...
	for (const [label, id] of cwv) {
		...
		const color = a.score >= 0.9 ? chalk.green : a.score >= 0.5 ? chalk.yellow : chalk.red;
	...
	if (opps.length === 0) console.log(chalk.dim("  (onemli firsat yok)"));
		...
		const saving = o.numericValue ? ` (${formatMs(o.numericValue)} kazanim)` : "";
	...
	else for (const a of a11yFails) console.log(`  ${chalk.red("XX")} ${a.title}`);
