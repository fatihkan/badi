...
let _trCache, _enCache;
async function loadTemplates(lang) {
	if (lang === "en") {
		if (!_enCache) _enCache = (await import("../templates/en.js")).contentTemplatesEN;
		return _enCache;
	}
	if (!_trCache) _trCache = (await import("../templates/tr.js")).contentTemplates;
	return _trCache;
}
...
export async function runIcerik(args) {
	...
	if (!subcommand || subcommand === "--help" || subcommand === "-h") {
		...
		console.log(`  ${chalk.cyan("badi icerik release-notes")}      App Store/Play Store release notes (--platform ios|android --version X.Y.Z)`);
		console.log("");
...
`;
	...
	// release-notes alt komutu — App Store/Play Store icin
	...
	// --lang, --sablon, --force, --platform parse
	...
	for (const lang of languages) {
		...
		// --platform appstore/playstore: CTA eki
