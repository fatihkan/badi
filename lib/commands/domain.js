...
function fetchSslCert(domain, port = 443) {
	return new Promise((resolve, reject) => {
		const socket = connect({ host: domain, port, servername: domain, rejectUnauthorized: false, timeout: 10000 });
		...
		socket.on("timeout", () => { socket.destroy(); reject(new Error("SSL baglanti zaman asimi")); });
...
async function runSsl(domain) {
	...
	console.log(`  Issuer:        ${chalk.cyan(cert.issuer.CN || cert.issuer.O || "?")}`);
	console.log(`  Alternative:   ${chalk.dim((cert.subjectaltname || "").replace(/DNS:/g, "").substring(0, 80))}`);
	console.log(`  Serial:        ${chalk.dim(cert.serialNumber)}`);

	console.log("");
	console.log(chalk.bold("Gecerlilik:"));
	console.log(`  Baslangic:     ${chalk.cyan(validFrom.toISOString().substring(0, 10))}`);
	console.log(`  Bitis:         ${chalk.cyan(validTo.toISOString().substring(0, 10))}`);
	const expColor = daysLeft < 14 ? chalk.bold.red : daysLeft < 30 ? chalk.bold.yellow : chalk.bold.green;
	console.log(`  Kalan:         ${expColor(daysLeft + " gun")} (${totalDays} gun toplam)`);
	...
	console.log("");
	console.log(issues === 0 ? chalk.bold.green("SSL saglikli!") : chalk.bold.yellow(`${issues} sorun tespit edildi.`));
}
...
async function runDns(domain) {
	...
	if (mx.length === 0) console.log(chalk.dim("  (kayit yok)"));
		...
		for (const r of mx) console.log(`  ${chalk.cyan(String(r.priority).padEnd(3))} ${r.exchange}`);
	}
	...
	const verification = txt.filter((t) => /google-site|ms-|facebook|apple-/.test(t));
	const other = txt.filter((t) => !spf.includes(t) && !dmarc.includes(t) && !dkim.includes(t) && !verification.includes(t));

	if (spf.length > 0) console.log(`  ${chalk.green("SPF")}    ${chalk.dim(spf[0].substring(0, 90))}`);
	else console.log(`  ${chalk.yellow("SPF")}    ${chalk.dim("tanimli degil")}`);

	for (const d of dmarc) console.log(`  ${chalk.green("DMARC")}  ${chalk.dim(d.substring(0, 90))}`);
	if (dmarc.length === 0) console.log(`  ${chalk.yellow("DMARC")}  ${chalk.dim("tanimli degil")}`);

	for (const v of verification) console.log(`  ${chalk.dim("Verify")} ${chalk.dim(v.substring(0, 90))}`);
	for (const t of other) console.log(`  ${chalk.dim("Other")}  ${chalk.dim(t.substring(0, 90))}`);

	console.log("");
	console.log(chalk.bold(`CAA (Certificate Authority, ${caa.length}):`));
	if (caa.length === 0) console.log(chalk.dim("  (kayit yok — herhangi bir CA sertifika verebilir)"));
	else for (const c of caa) console.log(`  ${chalk.cyan(c.issue || c.issuewild || c.iodef)}`);

	console.log("");
	console.log(chalk.bold("SOA:"));
	if (soa.length > 0) {
		const s = soa[0];
		console.log(`  Primary: ${chalk.cyan(s.nsname)}  Admin: ${chalk.dim(s.hostmaster)}`);
		console.log(`  Serial: ${chalk.cyan(s.serial)}  TTL: ${chalk.dim(s.minttl + "s")}`);
	}
	...
	if (mx.length > 0 && caa.length === 0) issues.push("CAA kaydi yok (her CA cert verebilir)");
	if (issues.length === 0) console.log(chalk.bold.green("  Temel guvenlik kayitlari mevcut"));
...
async function runWhois(domain) {
	...
	const created = extractField(raw, ["Creation Date", "Created On", "created", "registered"]);
	const updated = extractField(raw, ["Updated Date", "Last Updated", "changed"]);
	const expires = extractField(raw, ["Registry Expiry Date", "Registrar Registration Expiration Date", "Expiration Date", "Expiry Date", "paid-till"]);
	const status = raw.match(/^\s*Domain Status\s*:\s*(.+)$/gim)?.map((l) => l.split(":").slice(1).join(":").trim()) || [];
	const nameServers = raw.match(/^\s*Name Server\s*:\s*(.+)$/gim)?.map((l) => l.split(":").slice(1).join(":").trim().toLowerCase()) || [];

	console.log(chalk.bold("Tescil Bilgisi:"));
	console.log(`  Registrar:     ${chalk.cyan(registrar || "?")}`);
	console.log(`  Olusturulma:   ${chalk.cyan(created ? created.substring(0, 10) : "?")}`);
	console.log(`  Son guncelleme: ${chalk.cyan(updated ? updated.substring(0, 10) : "?")}`);

	if (expires) {
		const expDate = new Date(expires);
		const daysLeft = Math.floor((expDate - Date.now()) / 86400000);
		const expColor = daysLeft < 30 ? chalk.bold.red : daysLeft < 90 ? chalk.bold.yellow : chalk.bold.green;
		console.log(`  Expire:        ${chalk.cyan(expires.substring(0, 10))} (${expColor(daysLeft + " gun")})`);
	}
	...
	if (status.length > 0) {
		...
		for (const s of status.slice(0, 10)) console.log(`  ${chalk.dim(s.replace(/https?:\/\/\S+/, "").trim())}`);
	}

	// Guvenlik onerileri
	console.log("");
	console.log(chalk.bold("Guvenlik Onerileri:"));
	const hasTransferLock = status.some((s) => /clientTransferProhibited/i.test(s));
	const hasUpdateLock = status.some((s) => /clientUpdateProhibited/i.test(s));
	console.log(`  ${hasTransferLock ? chalk.green("OK") : chalk.yellow("!!")} Transfer Lock (domain hijacking koruma)`);
	console.log(`  ${hasUpdateLock ? chalk.green("OK") : chalk.yellow("!!")} Update Lock`);
}
...
export async function runDomain(args) {
	...
	if (!sub || sub === "--help" || sub === "-h") {
		...
		console.log("");
		console.log(`  ${chalk.cyan("badi ssl")} [domain]     SSL sertifika analizi (expire, TLS, cipher)`);
		console.log(`  ${chalk.cyan("badi dns")} [domain]     DNS kayitlari (A/AAAA/MX/TXT/SPF/DMARC/CAA)`);
		console.log(`  ${chalk.cyan("badi whois")} [domain]   Domain tescil + expire + transfer lock`);
		console.log("");
...
export async function runSslCmd(args) {
	...
	try { await runSsl(extractDomain(domain)); }
	catch (e) { console.error(chalk.red(`Hata: ${e.message}`)); process.exit(1); }
}
...
export async function runDnsCmd(args) {
	...
	try { await runDns(extractDomain(domain)); }
	catch (e) { console.error(chalk.red(`Hata: ${e.message}`)); process.exit(1); }
}
...
export async function runWhoisCmd(args) {
	...
	try { await runWhois(extractDomain(domain)); }
	catch (e) { console.error(chalk.red(`Hata: ${e.message}`)); process.exit(1); }
}
