...
export function checkDocsSync(ctx = {}) {
	...
	// 1) test-count internal consistency
	const counts = [];
	const badge = readme.match(/tests-(\d+)%20passing/);
	if (badge) counts.push({ where: "badge", n: Number(badge[1]) });
	const featRow = readme.match(/\*\*(\d+) passing tests\*\*/);
	if (featRow) counts.push({ where: "feature table", n: Number(featRow[1]) });
	const devLine = readme.match(/#\s*(\d+) tests across (\d+) suites/);
	if (devLine) counts.push({ where: "dev section", n: Number(devLine[1]) });
	...
	// 3) SECURITY.md supported-version row covers the target version
	if (ctx.targetVersion && existsSync(paths.security)) {
		...
		if (!sec.includes(want)) {
			problems.push(`SECURITY.md does not list ${want} as a supported version`);
		}
	}
