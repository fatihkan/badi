...
// Sibling of checkScoopManifest for the Homebrew channel. The formula skeleton
// carries a concrete download `url` (sha256 is filled at tap publish time by
// dist-publish.yml). The workflow re-renders the url version-agnostically and
// asserts the result, but the checked-in url is also what a manual tap install
// (dist/README.md) copies verbatim — and a stale url there ships the previous
// tarball. Same drift class as the scoop manifest, same gate: warn when the
// url does not reference the target version.
export function checkHomebrewFormula(ctx = {}) {
	const formulaPath = ctx.paths?.homebrew ?? "dist/homebrew/badi.rb";
	if (!existsSync(formulaPath)) return null; // optional distribution channel
	const version = ctx.targetVersion || readPackageJson()?.version;
	if (!version) return null;
	const body = readFileSync(formulaPath, "utf-8");
	const urlMatch = body.match(/^\s*url\s+"([^"]*)"/m);
	if (!urlMatch) {
		return {
			name: "homebrew-formula",
			label: "dist/homebrew/badi.rb has a url line",
			pass: false,
			level: "fail",
			hint: 'no url "..." line found in dist/homebrew/badi.rb',
		};
	}
	// Delimited tarball token, not a bare substring — `badi-11.3.2.tgz` must
	// not satisfy version `1.3.2` (prefix collision), same as the scoop check.
	if (!urlMatch[1].includes(`badi-${version}.tgz`)) {
		return {
			name: "homebrew-formula",
			label: "dist/homebrew/badi.rb in sync",
			pass: false,
			level: "warn",
			hint: `url does not reference ${version} (${urlMatch[1]}) — update dist/homebrew/badi.rb`,
		};
	}
	return {
		name: "homebrew-formula",
