...
function devDeps(args) {
	...
	// npm outdated
	try {
		const output = execFileSync(mgr, ["outdated", "--json"], { encoding: "utf-8", stdio: ["pipe", "pipe", "pipe"] });
		...
		if (patch.length > 0) {
			console.log(chalk.bold.green(`Patch (${patch.length}) — guvenli, otomatik yapilabilir:`));
			for (const p of patch) console.log(`  ${chalk.cyan(p.name.padEnd(30))} ${chalk.dim(p.cur)} -> ${chalk.green(p.latest)}`);
		}
		if (minor.length > 0) {
			console.log("");
			console.log(chalk.bold.yellow(`Minor (${minor.length}) — dikkatli, test gerekli:`));
			for (const p of minor) console.log(`  ${chalk.cyan(p.name.padEnd(30))} ${chalk.dim(p.cur)} -> ${chalk.yellow(p.latest)}`);
		}
		if (major.length > 0) {
			console.log("");
			console.log(chalk.bold.red(`Major (${major.length}) — break change olasi, manuel inceleme:`));
			for (const p of major) console.log(`  ${chalk.cyan(p.name.padEnd(30))} ${chalk.dim(p.cur)} -> ${chalk.red(p.latest)}`);
		}

		console.log("");
		console.log(chalk.bold("Onerilen komutlar:"));
		console.log(chalk.dim(`  ${mgr} update                    # patch + minor (lock file sinirlari)`));
		console.log(chalk.dim(`  ${mgr} install [paket]@latest    # major tekil`));
		console.log(chalk.dim("  npx npm-check-updates -u        # tum major'lari guncelle (dikkatli)"));
		...
		if (args.includes("--apply-patch") && patch.length > 0) {
			...
			try {
				execFileSync(mgr, ["update", ...patch.map((p) => p.name)], { stdio: "inherit" });
		...
		console.log(chalk.green("Tum bagimliliklar guncel veya outdated bilgisi alinamadi."));
	}
}
...
function devBundle() {
	...
	// Framework tespit
	const pkg = existsSync("package.json") ? JSON.parse(readFileSync("package.json", "utf-8")) : null;
	if (!pkg) {
	...
	if (found) {
		...
		console.log(`  Toplam: ${chalk.cyan((bytes / (1024 * 1024)).toFixed(2) + " MB")}`);
		...
		function collectFiles(dir) {
			for (const entry of readdirSync(dir, { withFileTypes: true })) {
				...
				if (entry.isDirectory()) collectFiles(full);
					...
					try { files.push({ path: full, size: statSync(full).size }); } catch { /* */ }
				}
			}
		}
		collectFiles(found);
		...
		for (const f of files.slice(0, 10)) {
			...
			const color = f.size > 500000 ? chalk.red : f.size > 200000 ? chalk.yellow : chalk.green;
			console.log(`  ${color(kb.padStart(8) + " KB")}  ${chalk.dim(f.path)}`);
		}

		console.log("");
		console.log(chalk.bold("Onerilen Araclar:"));
		if (hasWebpack) console.log(chalk.dim("  npx webpack-bundle-analyzer [stats.json]"));
...
function devDockerLint() {
	...
	const dockerfile = existsSync("Dockerfile") ? "Dockerfile" : existsSync("dockerfile") ? "dockerfile" : null;
	if (!dockerfile) {
	...
	lines.forEach((line, i) => {
		...
		if (upper.startsWith("FROM ")) {
			...
			if (/:latest/.test(trimmed) || !/:/.test(trimmed.split(" ")[1] || "")) {
				warn(n, "FROM ... :latest veya etiketsiz — versiyonu sabitleyin (guvenlik + reproducibility)");
			}
		}
		// USER
		...
		if (/^RUN .*apt-get install/i.test(trimmed) && !/--no-install-recommends/.test(trimmed)) {
		...
		if (/^RUN apt-get update/i.test(trimmed) && !/&&/.test(trimmed)) {
			warn(n, "apt-get update ile install'i ayni RUN'da birlestirin (cache busting)");
		}
		// ADD yerine COPY
		...
		if (/^EXPOSE\s+\d+/i.test(trimmed)) {
			const port = Number.parseInt(trimmed.split(/\s+/)[1]);
			if (port < 1024) warn(n, `EXPOSE ${port} — 1024 altinda root gerekir, mumkunse daha yuksek port`);
		}
	});

	// Global kontroller
	if (!hasUser) warn(0, "USER direktifi yok — root olarak calisiyor (guvenlik riski)");
	if (!hasHealthcheck) warn(0, "HEALTHCHECK tanimlanmamis — orchestration saglik kontrolu icin eklemeyi dusun");
...
function devEnvCheck() {
	...
	if (!exampleFile) {
		console.error(chalk.red("Hicbir .env.example/.env.template/.env.sample bulunamadi"));
		process.exit(1);
	}
	...
	if (extra.length > 0) {
		console.log("");
		console.log(chalk.bold.yellow(`Fazladan (${extra.length}) — .env.example'a ekleyin:`));
	...
	const placeholders = [];
	for (const [k, v] of actual) {
		if (/^(your[_-]?\w+|xxx|changeme|todo|tbd|placeholder|<.*>)$/i.test(v.replace(/['"]/g, ""))) {
			placeholders.push({ k, v });
		}
	}
	if (placeholders.length > 0) {
		console.log("");
		console.log(chalk.bold.red(`Placeholder deger (${placeholders.length}) — guncellenmemis:`));
		for (const { k, v } of placeholders) console.log(`  ${chalk.red("XX")} ${k}=${chalk.dim(v)}`);
...
async function devApiTest(args) {
	...
	if (!url) {
		...
		console.log("");
		console.log(`  ${chalk.cyan("badi dev api-test")} [url]                    GET istek`);
		console.log(`  ${chalk.cyan("badi dev api-test")} [url] --method POST      HTTP metod`);
		console.log(`  ${chalk.cyan("badi dev api-test")} [url] --body '{}'        JSON body`);
		console.log(`  ${chalk.cyan("badi dev api-test")} [url] --header 'K: V'    Header ekle`);
		console.log(`  ${chalk.cyan("badi dev api-test")} [url] --expect 200       Beklenen status`);
		console.log("");
		console.log(chalk.bold("Ornekler:"));
		console.log(`  badi dev api-test https://api.example.com/users`);
		console.log(`  badi dev api-test https://api.example.com/users --method POST --body '{"name":"X"}'`);
		return;
	}
	...
	for (let i = 1; i < args.length; i++) {
		...
		else if (args[i] === "--expect") expectStatus = Number.parseInt(args[++i]);
	}
	...
	if (Object.keys(headers).length > 0) {
		...
		for (const [k, v] of Object.entries(headers)) console.log(`  ${chalk.dim(k + ": " + v)}`);
		console.log("");
	}
	...
	try {
		...
		// Status
		const statusColor = res.status < 300 ? chalk.green : res.status < 400 ? chalk.yellow : chalk.red;
		console.log(chalk.bold("Yanit:"));
		console.log(`  Status:   ${statusColor(res.status + " " + res.statusText)}`);
		console.log(`  Sure:     ${chalk.cyan(duration + "ms")}`);
		console.log(`  Boyut:    ${chalk.cyan((text.length / 1024).toFixed(2) + " KB")}`);
		console.log(`  Content:  ${chalk.dim(res.headers.get("content-type") || "?")}`);
		...
		if (expectStatus !== null) {
			...
			if (res.status === expectStatus) {
				console.log(chalk.bold.green(`OK Beklenen status ${expectStatus} eslesti`));
			} else {
				console.log(chalk.bold.red(`FAIL Beklenen ${expectStatus}, alinan ${res.status}`));
				process.exit(1);
			}
		}
	} catch (e) {
...
export async function runDev(args) {
	...
	if (!sub || sub === "--help" || sub === "-h") {
		...
		console.log("");
		console.log(`  ${chalk.cyan("badi dev deps")}                  Bagimlilik guncelleme analizi`);
		console.log(`  ${chalk.cyan("badi dev deps --apply-patch")}    Patch guncellemelerini uygula`);
		console.log(`  ${chalk.cyan("badi dev bundle")}                Bundle size + framework tespit`);
		console.log(`  ${chalk.cyan("badi dev docker-lint")}           Dockerfile best practice`);
		console.log(`  ${chalk.cyan("badi dev env-check")}             .env dogrulama`);
		console.log(`  ${chalk.cyan("badi dev api-test")} [url]        HTTP endpoint tester`);
		console.log("");
		console.log(chalk.bold("Ornekler:"));
		console.log("  badi dev deps");
		console.log("  badi dev bundle");
		console.log("  badi dev api-test https://api.example.com/health --expect 200");
		return;
	}

	try {
		switch (sub) {
			case "deps": devDeps(args.slice(1)); break;
			case "bundle": devBundle(); break;
			case "docker-lint": devDockerLint(); break;
			case "env-check": devEnvCheck(); break;
			case "api-test": await devApiTest(args.slice(1)); break;
			default:
				console.error(chalk.red(`Bilinmeyen dev komutu: ${sub}`));
				console.log("Kullanim: badi dev [deps|bundle|docker-lint|env-check|api-test]");
				process.exit(1);
		}
	} catch (e) {
		console.error(chalk.red(`Hata: ${e.message}`));
