// `badi design` — Visual identity command. Wraps Google's @google/design.md
// CLI (npx) for lint/export and provides project-local init/show.
//
// Subcommands:
//   tasarim init [--example <name>] [--out PATH] [--force]
//   tasarim lint [--strict]
...
function parseFlags(args) {
	...
	for (let i = 0; i < args.length; i++) {
		const a = args[i];
		if (a === "--example") flags.ornek = args[++i];
...
function showHelp() {
	...
	console.log(
		`  ${chalk.cyan("badi design init")}                      Create a new DESIGN.md`,
	);
	console.log(
		`  ${chalk.cyan("badi design init --example <name>")}        Copy from an example template`,
	);
	console.log(
		`  ${chalk.cyan("badi design lint")}                      Validate DESIGN.md`,
	);
	console.log(
		`  ${chalk.cyan("badi design export --format tailwind")}   Generate Tailwind config`,
	);
	console.log(
		`  ${chalk.cyan("badi design export --format dtcg")}       Generate DTCG token JSON`,
	);
	console.log(
		`  ${chalk.cyan("badi design show --tokens")}              Frontmatter only (tokens)`,
	);
	console.log(
		`  ${chalk.cyan("badi design show --prose")}               Markdown body only (rationale)`,
	);
	console.log("");
	...
	console.log(`  --example <name>   ${EXAMPLES.join(" | ")}`);
...
For a skeleton start: badi design init --force
`;
	...
	console.log(chalk.dim("  Next: badi design lint"));
}
...
function subLint(flags) {
	...
	if (!existsSync(target)) {
		...
		console.log(chalk.dim("  First: badi design init"));
		process.exit(1);
	}
...
export async function runTasarim(args) {
	...
	switch (sub) {
		...
		default:
			...
			console.log(chalk.dim("  Help: badi design --help"));
			process.exit(1);
	}
}
