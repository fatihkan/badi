...
export function runDurum() {
	...
	if (!existsSync(workspaceBase)) {
		console.log(
			chalk.dim("No content generated yet. Start: badi content start"),
		);
		return;
	}
	...
	if (envanter.bugun === 0) {
		console.log(
			chalk.dim("INFO: No content generated today yet. badi content start"),
		);
	}
	console.log("");
	console.log(
		chalk.dim("For detailed status, use /content-status in Claude Code."),
	);
}
