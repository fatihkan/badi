...
export async function runTemplate(args) {
	...
	// English-only: parseLanguageFlag strips --lang from argv (no-op).
	...
	switch (subcommand) {
		...
		case "carousel":
			subdir = "icerikler";
			fileName = `${dateStr}-carousel-${konuSlug}.md`;
	...
	console.log(
		`  3. For the full interactive flow, use ${chalk.cyan(`/${subcommand === "post" ? "content-generate" : subcommand === "video" ? "content-video-script" : subcommand === "visual" ? "content-visual-brief" : subcommand === "calendar" ? "content-calendar" : subcommand === "carousel" ? "content-carousel" : "content-generate"}`)} in Claude Code`,
	);
}
