...
export async function runTemplate(args) {
	...
	console.log(
		`  3. For the full interactive flow, use ${chalk.cyan(`/${subcommand === "post" ? "icerik-uret" : subcommand === "video" ? "video-senaryo" : subcommand === "visual" ? "gorsel-brief" : subcommand === "calendar" ? "icerik-takvimi" : subcommand === "karousel" ? "karousel" : "icerik-uret"}`)} in Claude Code`,
	);
}
