...
export function runAra(args) {
	...
	if (showAraHelp || !query) {
		...
		console.log("");
		console.log(
			`  badi content search [query]           ${chalk.dim("Keyword search")}`,
		);
		console.log(
			`  badi content search [q] --platform X  ${chalk.dim("Platform filter")}`,
		);
		console.log(
			`  badi content search [q] --type post   ${chalk.dim("Type filter (post/carousel/video/visual)")}`,
		);
		console.log(
			`  badi content search [q] --last 30     ${chalk.dim("Last N days")}`,
		);
		console.log(
			`  badi content search [q] --hashtag X   ${chalk.dim("Hashtag search")}`,
		);
		console.log(
			`  badi content search [q] --format json ${chalk.dim("JSON output")}`,
		);
