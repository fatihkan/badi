...
export function runSablon(args) {
	...
	if (!sablonSub || sablonSub === "--help" || sablonSub === "-h") {
		...
		console.log("");
		console.log(
			`  badi icerik sablon olustur [name] --extends [type]  ${chalk.dim("Create new template")}`,
		);
		console.log(
			`  badi icerik sablon list                             ${chalk.dim("List templates")}`,
		);
		console.log(
			`  badi icerik sablon sil [name]                       ${chalk.dim("Delete template")}`,
		);
		console.log("");
		console.log(chalk.bold("Usage:"));
		console.log('  badi icerik post "topic" --sablon saas-launch');
		console.log("");
	...
	if (sablonSub === "olustur") {
		...
		if (!sablonName) {
			console.error(
				chalk.red(
					"Specify a template name: badi icerik sablon olustur [name] --extends [type]",
				),
			);
			process.exit(1);
		}
...
`;
		...
		console.log(
			`Usage: badi icerik ${extendsType} "topic" --sablon ${slugify(sablonName)}`,
		);
		return;
	}
	...
	if (sablonSub === "list") {
		if (!existsSync(sablonDir)) {
			...
			console.log(
				chalk.dim("Create: badi icerik sablon olustur [name] --extends post"),
			);
			return;
		}
	...
	console.log("Usage: badi icerik sablon [olustur|list|sil]");
	process.exit(1);
}
