...
export function runAc(args) {
	...
	if (!existsSync(workspaceBase)) {
		console.log(
			chalk.dim(
				'No workspace. Generate content first: badi content post "topic"',
			),
		);
		return;
	}
	...
	if (allFiles.length === 0) {
		...
		console.log(chalk.dim("For the file list: badi content list"));
		return;
	}
	...
	if (args.includes("--open") || args.includes("-o")) {
		...
		console.log(chalk.dim("To open in editor: badi content open --open"));
	}
