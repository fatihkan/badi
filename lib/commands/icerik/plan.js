...
export function runPlan() {
	...
	console.log(
		`  1. Create a detailed calendar: ${chalk.cyan(`badi icerik takvim "${formatDate(nextMonday).substring(0, 7)}"`)}`,
	);
	console.log("  2. Set a topic for each day (fill the calendar file)");
	console.log(
		`  3. Prepare the week's first content: ${chalk.cyan('badi icerik post "[topic]"')}`,
	);
	console.log("");
	console.log(
		chalk.dim(
			"For a detailed planning session, use /icerik-plan in Claude Code.",
		),
	);
}
