...
export function runPerf(args) {
	...
	if (perfSub === "--help" || perfSub === "-h") {
		...
		console.log("");
		console.log(
			`  badi icerik perf                      ${chalk.dim("Weekly summary (default)")}`,
		);
		console.log(
			`  badi icerik perf add [options]        ${chalk.dim("Add performance data")}`,
		);
		console.log(
			`  badi icerik perf list                 ${chalk.dim("List all records")}`,
		);
		console.log("");
	...
	if (perfSub === "add") {
		...
		if (!entry.file || !entry.platform) {
			...
			console.log(
				"Example: badi icerik perf add --file test.md --platform instagram --likes 100",
			);
			process.exit(1);
		}
	...
	if (perfSub === "list") {
		...
		if (entries.length === 0) {
			...
			console.log(
				chalk.dim(
					"Add data: badi icerik perf add --file X --platform Y --likes N",
				),
			);
			return;
		}
