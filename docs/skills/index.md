...
62 skill categories live in `.claude/skills-vault/`. Since v1.17, **none are auto-loaded** — opt in to only the ones you need. Since v1.20, an **auto-router** can inject matching skills per prompt without permanent activation (`badi skills auto on`).
...
Auto-loading 62 skill categories used to cost ~10–15k tokens per turn even when no skill was triggered. v1.17 moved them to a vault that Claude Code does **not** scan, then exposed an explicit picker.
