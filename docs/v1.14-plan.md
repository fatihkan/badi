...
any compatible AI coding agent. #57 is the flagship payload: it packages
...
- **Distribution channels.** Phase 2 item — confirm submission flow
  before publishing the README install instructions widely.

## v1.14.1 — design.md integration (#58)

### Sequence

1. **Phase 1: CLI wrapper** (~1 day). `lib/commands/tasarim.js` with `init` /
   `lint` / `export` / `show` subcommands. Each shells out to
   `npx @google/design.md@0.1.1` (pinned — package is alpha v0.1.1).
2. **Phase 2: `tasarim-kurator` agent** (~2 days). Reads the project
   `DESIGN.md`, answers token queries, delegates from `visual-director`.
3. **Phase 3: `design-tokens` skill** (~1 day). Injects DESIGN.md tokens into
