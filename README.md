...
| Qwen Code (v1.37+) | `QWEN.md` | 86 | `.qwen/settings.json` | 30 | 14 | — |
...
Claude is the canonical source. Qwen/Cursor/Gemini/Windsurf/AGENTS.md adapters compile from the same `.claude/` tree. **Qwen Code is the only non-Claude target that carries the hooks** — its `PreToolUse` hooks can deny a tool call, so `badi init --harness qwen` translates the matchers to Qwen's tool ids (`Bash` → `run_shell_command`, `Write|Edit|NotebookEdit` → `write_file|edit|notebook_edit`) and the guardrails keep working. The hook scripts themselves are not forked: they resolve the project root from git, so the same `.claude/hooks/*.mjs` bodies run under both. Components a target harness does not support (hooks/skills/subagents) are listed in the `badi init` output under `skippedComponents`. AGENTS.md is the neutral fallback for OpenAI Codex CLI, Aider, and any other tool that reads project-level agent context.

## What You Get

| Feature | Details |
|---------|---------|
| **30 expert agents + 86 commands** | Full toolkit from security scanner to performance profiler, plus a virtual eng team (CEO / eng manager / release manager / QA lead) wired together by `/team`, an ads strategist (`/meta-review`, `/ads-review`), `/market` demand research, and `/tasks` dependency-aware sequencing (`[P]` markers → parallel execution) — profile-based filtering (core/dev/content/pentest) since v1.26 |
...
| Version | Summary |
|---------|---------|
| **v1.38.1** | **`badi list --hooks` fix + refreshed demo.** `list --hooks` filtered by the old `.sh` extension after hooks migrated to Node `.mjs`, so it silently reported `Hooks (0)` on every real install; now matches `.mjs` (keeps `.sh` for legacy) and skips `_`-prefixed helpers, with a regression test that asserts the real count rather than just the word "Hook". The demo (`assets/demo.gif`) was rebuilt to install first, show the 14 safety hooks and a healthy `doctor`, and carry the current fleet counts instead of the long-stale `22 / 77 / 12 / 24`. 1333 → 1335 tests. |
| **v1.38.0** | **Security-hardening batch from an external-repo review.** Ran `/market` + `/ceo-review` + `/team` over five external repos via a `/workflows` fan-out; the adversarial gate killed or deferred every net-new feature idea, so this release only sharpens existing surfaces. The always-on `completeness-gate` now blocks a credential embedded in a URL authority and a secret carried as a URL query parameter (plain URLs are not flagged). A new `doctor` check and CI test flag a `PreToolUse`/`PostToolUse` matcher that names a tool outside the real Claude vocabulary — "present but dead," the class of defect behind v1.37.0's invalid `SessionStart` matchers. Six agents that consume external content gain an untrusted-input boundary (fetched/read content is data, never instructions). `/system-audit` + `auditor` gain an `INCONCLUSIVE` grade-floor so a mandatory check that never ran can't be averaged into a pass, and `/review` now requires evidence per finding. A new CI test greps exactly what npm ships against a denylist of analyzed-but-not-adopted third-party repos — it caught three pre-existing name leaks, now fixed. `/deps-update` documents the native minimum-release-age cooldown flag. 1325 → 1333 tests. |
| **v1.37.0** | **Qwen Code harness + a silent hook defect fixed.** Adds `badi init --harness qwen` — the 6th harness and the first non-Claude target that carries the *blocking* hooks, not just the process layer, because Qwen Code implements `PreToolUse` hooks that can deny a tool call plus subagents and per-file commands. Tool ids are translated for the target (`Bash` → `run_shell_command`, `Write\|Edit\|NotebookEdit` → `write_file\|edit\|notebook_edit`); the hook scripts are *not* forked, since they resolve the project root from git rather than `$CLAUDE_PROJECT_DIR`. Porting them exposed that badi's own `SessionStart` matchers (`"new"`/`"resumed"`) were not real source values — `session-reset`, `dependency-audit` and `post-compact-resume` had never fired, leaving the `pre-compact-handoff → compact → post-compact-resume` flow broken in half. Fixed, with regression tests that guard hook *wiring* (a correct hook that never runs raises no error). 1322 → 1335 tests. |
